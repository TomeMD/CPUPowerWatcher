...
if [ "$OS_VIRT" == "docker" ]; then
	...
	sudo apptainer instance start -B "${GLANCES_HOME}"/etc:/etc/glances --env "GLANCES_OPT=-q --export influxdb2 --time 2" "${GLANCES_HOME}"/glances.sif "${INFLUXDB_HOST}" "${INFLUXDB_BUCKET}"
