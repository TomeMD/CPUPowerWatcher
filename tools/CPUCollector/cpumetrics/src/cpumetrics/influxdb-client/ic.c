...
char influx_database[256+1];		/* the influxdb database (bucket) */
char influx_token[256+1];			/* the influxdb token */
char influx_org[64+1];				/* the influxdb organization */
...
void ic_influx_database(char *host, long port, char *db, char *org, char *token) /* note: converts influxdb hostname to ip address */
{
	struct hostent *he;
	...
	strncpy(influx_token,token,256);
	strncpy(influx_org,org,64);

	if(host[0] <= '0' && host[0] <='9') {
		DEBUG fprintf(stderr,"ic_influx(ipaddr=%s,port=%ld,database=%s))\n",host,port,db);
		strncpy(influx_ip,host,16);
	} else {
		DEBUG fprintf(stderr,"ic_influx_by_hostname(host=%s,port=%ld,database=%s,org=%s,token=%s))\n",host,port,db,org,token);
...
{
    if(output == NULL) {			/* First time create the buffer */
...
void ic_push()
{
    ...
    if (influx_port) {
        ...
        if (create_socket() == 1) {
            ...
            sprintf(buffer, "POST /api/v2/write?org=%s&bucket=%s HTTP/1.1\r\nHost: %s:%ld\r\nAuthorization: Token %s\r\nContent-Type: text/plain; charset=utf-8\r\nContent-Length: %ld\r\n\r\n",
            influx_org, influx_database, influx_hostname, influx_port, influx_token, output_char);
            DEBUG fprintf(stderr, "buffer size=%ld\nbuffer=<%s>\n", strlen(buffer), buffer);
            if ((ret = write(sockfd, buffer, strlen(buffer))) != (int) strlen(buffer)) {
            ...
            for (i = 0; i < 8192; i++) /* empty the buffer */
                result[i] = 0;
